...
int main(int argc, char **argv)
{
    ...
    int sockfd, n;
    int sendbytes;
    // sockaddr_in is a struct specific to IP-based communication, sockaddr is a general socket
    ...
    // host to network, short: handles conversion from host byte order to network standard byte order
    servaddr.sin_port = htons(SERVERPORT);
    
    // takes string IP address and converts it to binary representation and stores it in servadd.sin_addr
    if (inet_pton(AF_INET, argv[1], &servaddr.sin_addr) <= 0) {
        printf("inet_pton error for %s \n", argv[1]);
        exit(1);
    }

    // connects the socket referred to by sockfd to the address specified by servaddr
